...
set_max_delay -from  [get_cells -hier -nocase -regexp {.*I_REGISTERS/EXTENDED_MULTICAST.I_MULTICAST_MEM/.*} -filter {IS_SEQUENTIAL=="1"}] -to [get_cells -hier -nocase -regexp {.*I_REGISTERS/IP2Bus_Data_reg.*}  -filter {IS_SEQUENTIAL=="1"}] 8 -datapath_only
